...
// Records that got timeout errors (from ~record 176 onwards)
const missedRecords = [
...
async function scrape() {
  console.log('🔍 Re-scraping missed 2026 records...');
  console.log(`Records to check: ${missedRecords.length}\n`);
  ...
  for (let i = 0; i < missedRecords.length; i++) {
    const recordingNumber = missedRecords[i];
    process.stdout.write(`[${i + 1}/${missedRecords.length}] ${recordingNumber}... `);
  ...
  if (sheriffDeeds.length > 0) {
    console.log(`\n🎯 Found ${sheriffDeeds.length} MORE Sheriff Deed(s)!`);
    sheriffDeeds.forEach(d => console.log(`   - ${d.recordingNumber}: ${d.caseNumber || 'N/A'}`));
  } else {
    console.log('\n✅ No additional Sheriff Deeds in missed records');
  }
